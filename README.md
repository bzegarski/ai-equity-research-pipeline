...
**Sample output** (download and open in a browser):

- [`sample-report/PAYX_full_report.html`](sample-report/PAYX_full_report.html) — Paychex: a mature, profitable cash generator, the company shape the valuation framework is built for. Verdict: PASS at $93 against a ~$71 base intrinsic — a conventional value-discipline answer.
- [`sample-report/CRWD_full_report.html`](sample-report/CRWD_full_report.html) — CrowdStrike: a high-growth, SBC-heavy software name. The conservative owner-earnings framework prices businesses like this far below market by design, and the verdict is a PASS with the full reasoning, price ladder, and disconfirmation register shown. Read the valuation-posture note under Known limitations before judging the numbers.
